...
router.post("/", middleware_1.isAuthenticated, (0, multer_utils_1.multerUploadFile)().array("attachments"), (0, middleware_1.isValid)(post_validation_1.createPostSchema), async (req, res, next) => {
    ...
    const post = await post_service_1.default.create(req.body, req.user._id, req.files);
...
router.patch("/:postId", middleware_1.isAuthenticated, (0, multer_utils_1.multerUploadFile)().array("attachments"), (0, middleware_1.isValid)(post_validation_1.createPostSchema), async (req, res, next) => {
    const hasFile = !!req.file || (req.files && req.files.length > 0);
    const hasContent = !!req.body.content;
    if (!hasFile && !hasContent) {
        throw new common_1.BadRequestException("content or attachment is required");
    }
    const post = await post_service_1.default.update(req.body, req.params.postId, req.user._id, req.files);
    return (0, common_1.successResponse)({ res, message: "done", data: post });
});
