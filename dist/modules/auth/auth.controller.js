...
router.post("/login", (0, middleware_1.isValid)(auth_validation_1.loginSchema), async (req, res, next) => {
    ...
    return (0, common_1.successResponse)({ res, success: true, message: "done", data: { accessToken, refreshToken } });
});
...
router.patch("/reset-password", middleware_1.isAuthenticated, (0, middleware_1.isValid)(auth_validation_1.resetPasswordSchema), async (req, res, next) => {
    await auth_service_1.authService.resetPassword(req.body, req.user.email, req.payload);
...
router.patch("/change-password", middleware_1.isAuthenticated, (0, middleware_1.isValid)(auth_validation_1.changePasswordSchema), async (req, res, next) => {
    await auth_service_1.authService.changePassword(req.body, req.user._id, req.user.email, req.payload);
    return (0, common_1.successResponse)({ res, message: "password changed successfully" });
});
router.get("/refresh-token", async (req, res, next) => {
    const { accessToken, refreshToken } = await auth_service_1.authService.refreshToken(req.headers.authorization, req.body);
    return (0, common_1.successResponse)({ res, message: "done", data: { accessToken, refreshToken } });
});
router.patch("/log-out-from-all-devices", middleware_1.isAuthenticated, async (req, res, next) => {
    await auth_service_1.authService.logOutFromAllDevices(req.user._id);
    return (0, common_1.successResponse)({ res, message: "done" });
});
router.post("/log-out", middleware_1.isAuthenticated, async (req, res, next) => {
    await auth_service_1.authService.logOut(req.payload, req.user.email, req.body, req.user._id);
    return (0, common_1.successResponse)({ res, message: "done" });
});
